...
export const alertCargo = async (title, data) => {
    ...
    let permisos = ""
    for (permisos in data.permisos){
        const dataPermiso=`
            <p class="m-0 mb-2 text-start"><b>Nombre: </b>${permisos.nombre}</p>
            <p class="m-0 mb-2 text-start"><b>Nombre: </b>${permisos.descripcion}</p>
        `
        permisos=permisos + dataPermiso
    }
    return Swal.fire({
        title: `${title}`,
        showCloseButton: true,
        ...
        <div class='p-2 d-flex flex-column justify-content-start align-items-start'>
            ...
            ${
                data.permisos?
                `<p class="m-0 mb-2 text-start"><b>Permisos: </b></p>
                ${permisos}
                `
                :
                ''
